    ...
    if (this.fill || this.border > 0) {
      for (let y = 0; y < position.height; y++) {
        for (let x = 0; x < position.width; x++) {
          ...
          let value = (Math.pow(x - xCenter, 2) / Math.pow(widthHalf, 2)) + (Math.pow(y - yCenter, 2) / Math.pow(heightHalf, 2)) <= 1;
          if (!this.fill) {
            value = value && ((Math.pow(x - xCenter, 2) / Math.pow(widthHalfBorder, 2)) + (Math.pow(y - yCenter, 2) / Math.pow(heightHalfBorder, 2)) >= 1);
          }
